...
const PredictionsPage = () => {
  ...
  const filteredPredictions = predictions.filter((prediction) => {
    const matchesSearch =
      prediction.learnerName.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  return (
    <div className="p-6 bg-slate-900 min-h-screen">
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-2xl font-bold text-white">AI Performance Predictions</h1>
          <p className="text-slate-300">AI-powered insights and predictions for learner performance</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <div className="bg-slate-800 p-4 rounded-lg border border-slate-700">
          <div className="flex items-center">
            ...
            <div className="ml-3">
              <p className="text-sm text-slate-300">Total Predictions</p>
        ...
        <div className="bg-slate-800 p-4 rounded-lg border border-slate-700">
          <div className="flex items-center">
            ...
            <div className="ml-3">
              ...
              <p className="text-2xl font-bold text-white">{predictions.filter((p) => p.confidence >= 0.8).length}</p>
            </div>
          </div>
        </div>
        <div className="bg-slate-800 p-4 rounded-lg border border-slate-700">
        ...
        <div className="bg-slate-800 p-4 rounded-lg border border-slate-700">
          <div className="flex items-center">
            ...
            <div className="ml-3">
              ...
              <p className="text-2xl font-bold text-white">{predictions.filter((p) => p.status === "active").length}</p>
            </div>
          </div>
        </div>
      </div>
          ...
          <table className="w-full">
            ...
            <tbody className="bg-slate-800 divide-y divide-slate-700">
              {filteredPredictions.map((prediction) => (
                <tr key={prediction.id} className="hover:bg-slate-700">
                  <td className="px-6 py-4">
                    <div className="flex items-center">
                      {getPredictionTypeIcon(prediction.predictionType)}
                      <div className="ml-3">
                        <div className="text-sm font-medium text-white">{prediction.learnerName}</div>
                        <div className="text-sm text-slate-300">{prediction.courseName}</div>
                        <div className="text-xs text-slate-400 mt-1">Updated: {prediction.lastUpdated}</div>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-slate-700 text-slate-200 capitalize">
                  ...
                  <td className="px-6 py-4">
                    <span
                      className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getRiskLevelColor(prediction.riskLevel)} capitalize`}
                    >
                  ...
                  <td className="px-6 py-4">
                    <div className="text-sm">
                      <div className="text-white">{prediction.currentValue}</div>
                      <div className="text-slate-300">→ {prediction.predictedValue}</div>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    <span
                      className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getStatusColor(prediction.status)} capitalize`}
                    >
                      {prediction.status}
                    </span>
                  </td>
      ...
      <div className="mt-6 bg-slate-800 rounded-lg border border-slate-700 p-6">
        <h3 className="text-lg font-medium text-white mb-4">Key Prediction Factors</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <h4 className="font-medium text-slate-200 mb-2">Positive Factors</h4>
          ...
          <div>
            <h4 className="font-medium text-slate-200 mb-2">Risk Factors</h4>
            <ul className="space-y-1 text-sm text-slate-300">
