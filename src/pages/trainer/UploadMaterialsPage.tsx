...
const UploadMaterialsPage: React.FC = () => {
  ...
  const stats: UploadStats = {
    ...
    totalSize:
      uploads
        .reduce((total, upload) => {
          const size = Number.parseFloat(upload.fileSize.split(" ")[0])
          return total + size
        }, 0)
        .toFixed(1) + " MB",
  }
  ...
  return (
    <div className="p-6 space-y-6 bg-slate-900 min-h-screen">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-white">Upload Materials</h1>
          <p className="text-slate-300">Upload and manage training materials and resources</p>
        </div>
        <button
      ...
      <div className="bg-slate-800 rounded-lg border border-slate-700 p-6">
        <h3 className="text-lg font-semibold text-white mb-4">Upload New Materials</h3>
        ...
        >
          ...
          <p className="text-lg font-medium text-white mb-2">Drag and drop files here, or click to select</p>
        ...
        {selectedFiles.length > 0 && (
          <div className="mt-6">
            <h4 className="text-sm font-medium text-white mb-3">Selected Files ({selectedFiles.length})</h4>
            ...
            <div className="mt-4 flex gap-3">
              ...
              <button
                ...
                className="bg-slate-600 text-white px-4 py-2 rounded-lg hover:bg-slate-500"
              >
                Clear All
              </button>
            </div>
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-slate-700">
            ...
            <tbody className="bg-slate-800 divide-y divide-slate-700">
              {uploads.map((upload) => (
                <tr key={upload.id} className="hover:bg-slate-700">
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      {getFileTypeIcon(upload.fileType)}
                      <div>
                        <div className="text-sm font-medium text-white">{upload.fileName}</div>
                        <div className="text-sm text-slate-300">{upload.fileSize}</div>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    <div className="space-y-2">
                      <div className="text-sm text-white">{upload.category}</div>
                  ...
                  <td className="px-6 py-4">
                    <div className="text-sm text-white">
                      <div className="text-slate-300">{upload.uploadDate}</div>
                      <div className="text-xs text-slate-400 mt-1">{upload.isPublic ? "Public" : "Private"}</div>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    <div className="space-y-2">
                      <span
                        className={`inline-flex items-center px-2 py-1 text-xs font-semibold rounded-full ${getStatusColor(upload.status)}`}
                      >
                  ...
                  <td className="px-6 py-4">
                    <div className="flex space-x-2">
                      ...
                      <button onClick={() => removeUpload(upload.id)} className="text-red-400 hover:text-red-300">
