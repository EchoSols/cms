...
import { BookOpen, Users, TrendingUp, CheckCircle, Search, Plus, Eye, Edit, Trash2, Copy } from "lucide-react"
...
const ProgramsPage: React.FC = () => {
  ...
  const renderStars = (rating: number) => {
    return Array.from({ length: 5 }, (_, i) => (
      <div key={i} className={`w-4 h-4 ${i < Math.floor(rating) ? "text-yellow-400" : "text-gray-300"}`}>
        ★
      </div>
    ))
  }
    ...
    <div className="p-6 space-y-6 bg-slate-900 min-h-screen">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-300">Manage and create comprehensive training programs for learners</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-slate-800 p-4 rounded-lg border border-slate-700">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-slate-300">Active Programs</p>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-purple-400">
                {Math.round(
                  mockTrainingPrograms.reduce((sum, p) => sum + p.completionRate, 0) / mockTrainingPrograms.length,
                )}
                %
              </p>
            </div>
        ...
        <div className="p-6">
          ...
          <div className="mb-6">
            <div className="flex flex-col md:flex-row gap-4">
              ...
              <select
                ...
                className="px-4 py-2 bg-slate-700 border border-slate-600 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent text-white"
              >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredPrograms.map((program) => (
              <div
                key={program.id}
                className="bg-slate-800 border border-slate-700 rounded-lg p-6 hover:shadow-md transition-shadow"
              >
                <div className="flex justify-between items-start mb-4">
                  <div className="flex gap-2">
                    <span
                      className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getCategoryColor(program.category)}`}
                    >
                      {program.category.replace("-", " ")}
                    </span>
                    <span
                      className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getDifficultyColor(program.difficulty)}`}
                    >
                      {program.difficulty}
                    </span>
                  </div>
                ...
                <p className="text-sm text-slate-300 mb-4 line-clamp-2">{program.description}</p>
                ...
                <div className="space-y-2 mb-4">
                  ...
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="text-white capitalize">{program.type}</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-slate-400">Completion Rate:</span>
                    <span className="text-white">{program.completionRate}%</span>
                  </div>
                </div>
                ...
                <div className="mb-4">
                  <div className="flex flex-wrap gap-1">
                    {program.tags.slice(0, 3).map((tag, index) => (
                      <span key={index} className="inline-flex px-2 py-1 text-xs bg-slate-700 text-slate-300 rounded">
                        {tag}
                      </span>
                    ))}
                    {program.tags.length > 3 && (
                      <span className="inline-flex px-2 py-1 text-xs bg-slate-700 text-slate-300 rounded">
                        +{program.tags.length - 3}
                      </span>
                    )}
                  </div>
                ...
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <span className="text-sm text-slate-400">({program.rating})</span>
                </div>
          ...
          {filteredPrograms.length === 0 && (
            <div className="text-center py-12">
              ...
              <h3 className="mt-2 text-sm font-medium text-white">No programs found</h3>
