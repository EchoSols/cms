...
const SkillAssessmentsPage = () => {
  ...
  const quickStats = [
    ...
    },
    {
      ...
      value: assessments.reduce((sum, a) => sum + a.totalAttempts, 0),
      icon: Users,
  ...
  const handleStatusChange = (assessmentId: string, newStatus: string) => {
    setAssessments((prev) => prev.map((a) => (a.id === assessmentId ? { ...a, status: newStatus as any } : a)))
  }
  ...
  return (
    <div className="p-6 space-y-6 bg-slate-900 min-h-screen">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-white">Skill Assessments</h1>
          <p className="text-slate-300">Create and manage skill assessments for learners</p>
        </div>
        <div className="flex items-center gap-3">
          <button className="px-4 py-2 bg-slate-700 text-slate-300 border border-slate-600 rounded-lg hover:bg-slate-600 transition-colors flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {filteredAssessments.map((assessment) => (
          <div
            key={assessment.id}
            className="bg-slate-800 p-4 rounded-lg border border-slate-700 hover:bg-slate-750 transition-colors"
          >
            <div className="flex items-start justify-between mb-3">
              <div className="flex-1">
                <h3 className="font-medium text-white">{assessment.title}</h3>
                <p className="text-sm text-slate-400 line-clamp-2">{assessment.description}</p>
              </div>
              <div className="flex flex-col items-end gap-2">
                ...
                <span
                  className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getDifficultyColor(assessment.difficulty)}`}
                >
            ...
            <div className="space-y-2 mb-4">
              <div className="flex items-center text-sm">
                ...
                <span className="text-white">{assessment.skillArea}</span>
              </div>
              <div className="flex items-center text-sm">
                <CheckSquare className="w-4 h-4 text-slate-400 mr-2" />
                <span className="text-slate-300">{assessment.questionCount} questions</span>
              </div>
              <div className="flex items-center text-sm">
                <Clock className="w-4 h-4 text-slate-400 mr-2" />
                <span className="text-slate-300">{getTimeText(assessment.timeLimit)}</span>
              </div>
              <div className="flex items-center text-sm">
                <Target className="w-4 h-4 text-slate-400 mr-2" />
                <span className="text-slate-300">Pass: {assessment.passingScore}%</span>
              </div>
            </div>
            ...
            <div className="mb-4">
              ...
              <div className="text-xs text-slate-400 mt-1">{assessment.totalAttempts} attempts</div>
            </div>

            <div className="flex items-center justify-between">
              <div className="text-xs text-slate-400">Instructor: {assessment.instructor}</div>
      ...
      {showDetailModal && selectedAssessment && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-slate-800 rounded-lg p-6 max-w-4xl w-full mx-4 max-h-[90vh] overflow-y-auto border border-slate-700">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
              <div>
                ...
                <div className="space-y-2">
                  <div>
                    ...
                    <span className="ml-2 text-sm text-white">{selectedAssessment.title}</span>
                  </div>
                  <div>
                    <span className="text-sm font-medium text-slate-400">Description:</span>
                    <span className="ml-2 text-sm text-white">{selectedAssessment.description}</span>
                  </div>
                  <div>
                    <span className="text-sm font-medium text-slate-400">Status:</span>
                    <span
                      className={`ml-2 inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getStatusColor(selectedAssessment.status)}`}
                    >
                  ...
                  <div>
                    ...
                    <span
                      className={`ml-2 inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getDifficultyColor(selectedAssessment.difficulty)}`}
                    >
              ...
              <div>
                ...
                <div className="space-y-2">
                  <div>
                    <span className="text-sm font-medium text-slate-400">Skill Area:</span>
                    <span className="ml-2 text-sm text-white">{selectedAssessment.skillArea}</span>
                  </div>
                  <div>
                    <span className="text-sm font-medium text-slate-400">Questions:</span>
                    <span className="ml-2 text-sm text-white">{selectedAssessment.questionCount}</span>
                  </div>
                  <div>
                    <span className="text-sm font-medium text-slate-400">Time Limit:</span>
                    <span className="ml-2 text-sm text-white">{getTimeText(selectedAssessment.timeLimit)}</span>
                  </div>
                  <div>
                    <span className="text-sm font-medium text-slate-400">Passing Score:</span>
                    <span className="ml-2 text-sm text-white">{selectedAssessment.passingScore}%</span>
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="space-y-2">
                  <div>
                    <span className="text-sm font-medium text-slate-400">Total Attempts:</span>
                    <span className="ml-2 text-sm text-white">{selectedAssessment.totalAttempts}</span>
                  </div>
                  <div>
                  ...
                  <div>
                    <span className="text-sm font-medium text-slate-400">Instructor:</span>
                    <span className="ml-2 text-sm text-white">{selectedAssessment.instructor}</span>
                  </div>
                </div>
              </div>

              <div>
                <h3 className="font-medium text-white mb-3">Target Audience & Tags</h3>
                <div className="space-y-2">
                  <div>
                    ...
                    <div className="ml-2 mt-1">
                      {selectedAssessment.targetAudience.map((audience, index) => (
                        <span
                          key={index}
                          className="inline-block bg-slate-700 text-slate-300 text-xs px-2 py-1 rounded mr-1 mb-1"
                        >
                          {audience}
                        </span>
                      ))}
                  ...
                  <div>
                    ...
                    <div className="ml-2 mt-1">
                      {selectedAssessment.tags.map((tag, index) => (
                        <span
                          key={index}
                          className="inline-block bg-blue-900 text-blue-300 text-xs px-2 py-1 rounded mr-1 mb-1"
                        >
                          {tag}
                        </span>
                      ))}
            ...
            <div className="bg-slate-700 p-4 rounded-lg mb-6">
              ...
              <div className="h-32 bg-slate-800 border border-slate-600 rounded flex items-center justify-center">
