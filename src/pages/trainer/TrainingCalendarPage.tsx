...
const TrainingCalendarPage = () => {
  ...
  const quickStats = [
    ...
    },
    {
      ...
      value: sessions.reduce((sum, s) => sum + s.currentParticipants, 0),
      icon: Users,
    ...
    <div className="p-6 space-y-6 bg-slate-900 min-h-screen">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-slate-300">Manage and schedule training sessions</p>
        </div>
        <div className="flex items-center gap-3">
          <button className="bg-slate-700 text-slate-300 px-4 py-2 rounded-lg hover:bg-slate-600 flex items-center gap-2">
          ...
          <div className="flex gap-3">
            ...
            <div className="flex border border-slate-600 rounded-lg">
              <button
                ...
                className={`px-3 py-2 ${viewMode === "calendar" ? "bg-blue-600 text-white" : "bg-slate-700 text-slate-300"}`}
              >
                Calendar
              </button>
              <button
        ...
        <div className="bg-slate-800 rounded-lg border border-slate-700 p-6">
          ...
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="flex items-center gap-2">
              <button onClick={() => navigateWeek("prev")} className="p-2 text-slate-400 hover:text-slate-300">
              ...
              <button onClick={() => navigateWeek("next")} className="p-2 text-slate-400 hover:text-slate-300">
          ...
          <div className="grid grid-cols-7 gap-px bg-slate-600 rounded-lg overflow-hidden">
            ...
            {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((day, index) => (
              <div key={index} className="bg-slate-700 p-3 text-center">
            ...
            {getCurrentWeekDates().map((date, index) => {
              ...
              return (
                <div key={index} className={`bg-slate-800 min-h-32 p-2 ${isToday ? "bg-blue-900/30" : ""}`}>
                  <div className={`text-sm font-medium mb-2 ${isToday ? "text-blue-400" : "text-white"}`}>
                    {date.getDate()}
                  </div>
                  ...
                  <div className="space-y-1">
                    {daySessions.map((session) => (
                      ...
                      >
                        ...
                        <div className="text-slate-300">{formatTime(session.startTime)}</div>
              ...
              <tbody className="bg-slate-800 divide-y divide-slate-700">
                {filteredSessions.map((session) => (
                  <tr key={session.id} className="hover:bg-slate-700">
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div>
                        ...
                        <div className="text-sm text-slate-300">
                          {session.program} - {session.course}
                        </div>
                        <div className="text-sm text-slate-300">Instructor: {session.instructor}</div>
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span
                        className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getTypeColor(session.type)}`}
                      >
                        {session.type.charAt(0).toUpperCase() + session.type.slice(1)}
                      </span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-white">{session.location}</td>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-slate-800 rounded-lg p-6 max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto border border-slate-700">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <h3 className="font-medium text-white mb-3">Session Information</h3>
                <div className="space-y-2">
                  <div>
                    <span className="text-sm font-medium text-slate-300">Title:</span>
                    <span className="ml-2 text-sm text-white">{selectedSession.title}</span>
                  </div>
                  <div>
                    <span className="text-sm font-medium text-slate-300">Type:</span>
                    <span
                      className={`ml-2 inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getTypeColor(selectedSession.type)}`}
                    >
                  ...
                  <div>
                    ...
                    <span
                      className={`ml-2 inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getStatusColor(selectedSession.status)}`}
                    >
                  ...
                  <div>
                    ...
                    <span className="ml-2 text-sm text-white">{selectedSession.description}</span>
                  </div>
                </div>
              </div>

              <div>
                <h3 className="font-medium text-white mb-3">Schedule & Location</h3>
                <div className="space-y-2">
                  <div>
                  ...
                  <div>
                    ...
                    <span className="ml-2 text-sm text-white">{selectedSession.duration} minutes</span>
                  </div>
                  <div>
                    <span className="text-sm font-medium text-slate-300">Location:</span>
                    <span className="ml-2 text-sm text-white">{selectedSession.location}</span>
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="space-y-2">
                  <div>
                    ...
                    <span className="ml-2 text-sm text-white">{selectedSession.program}</span>
                  </div>
                  <div>
                    <span className="text-sm font-medium text-slate-300">Course:</span>
                    <span className="ml-2 text-sm text-white">{selectedSession.course}</span>
                  </div>
                  <div>
                    <span className="text-sm font-medium text-slate-300">Instructor:</span>
                    <span className="ml-2 text-sm text-white">{selectedSession.instructor}</span>
                  </div>
                </div>
              </div>

              <div>
                <h3 className="font-medium text-white mb-3">Participation</h3>
                <div className="space-y-2">
                  <div>
                    <span className="text-sm font-medium text-slate-300">Current:</span>
                    <span className="ml-2 text-sm text-white">{selectedSession.currentParticipants}</span>
                  </div>
                  <div>
                    <span className="text-sm font-medium text-slate-300">Maximum:</span>
                    <span className="ml-2 text-sm text-white">{selectedSession.maxParticipants}</span>
                  </div>
                  <div>
            ...
            <div className="mt-6">
              ...
              <div className="flex flex-wrap gap-2">
                {selectedSession.tags.map((tag, index) => (
                  <span key={index} className="inline-block bg-slate-700 text-slate-300 text-xs px-2 py-1 rounded">
                    {tag}
                  </span>
                ))}
              </div>
