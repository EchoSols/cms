...
const mockMessages: Message[] = [
  {
    ...
    content:
      "Hi Sarah, I have a question about the project scope document assignment. Could you clarify the requirements for the stakeholder analysis section?",
    category: "Support",
    priority: "Medium",
    status: "Unread",
  ...
  },
  {
    ...
    content:
      "I'm experiencing issues accessing the video lectures. The page keeps loading indefinitely. Can you help me resolve this?",
    category: "Technical",
    priority: "High",
    status: "Read",
  ...
  },
  {
    ...
    content:
      "The Agile course is great! I was wondering if you could recommend additional resources for Scrum methodology.",
    category: "Feedback",
    priority: "Low",
    status: "Replied",
...
const InstructorMessagesPage: React.FC = () => {
  ...
  const stats: MessageStats = {
    ...
    responseRate: Math.round(
      (mockMessages.filter((msg) => msg.status === "Replied").length / mockMessages.length) * 100,
    ),
  ...
  return (
    <div className="p-6 space-y-6 bg-slate-900 min-h-screen">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-white">Instructor Messages</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
        ...
        <div className="bg-slate-800 p-4 rounded-lg border border-slate-700">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-slate-300">Unread</p>
              <p className="text-2xl font-bold text-blue-400">{stats.unreadCount}</p>
            </div>
          ...
          <table className="min-w-full divide-y divide-slate-700">
            ...
            <tbody className="bg-slate-800 divide-y divide-slate-700">
              {filteredMessages.map((message) => (
                <tr
                  key={message.id}
                  className={`hover:bg-slate-700 ${message.status === "Unread" ? "bg-blue-900/20" : ""}`}
                >
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          {message.attachments.length > 0 && <span className="text-xs text-slate-400">📎</span>}
                        </div>
                        <p className="text-sm text-white mt-1">{message.subject}</p>
                        <p className="text-sm text-slate-300 mt-1 line-clamp-2">{message.content}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm text-white">{message.category}</span>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    <span
                  ...
                  <td className="px-6 py-4">
                    <div className="space-y-2">
                      ...
                      <div className="text-xs text-slate-400">{message.sentDate}</div>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    <div className="flex space-x-2">
                      <button onClick={() => setSelectedMessage(message)} className="text-blue-400 hover:text-blue-300">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <button className="p-4 border border-slate-600 rounded-lg hover:bg-slate-700 text-left">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h4 className="font-medium text-white">Bulk Message</h4>
                <p className="text-sm text-slate-300">Send message to multiple learners</p>
              </div>
            </div>
          </button>
          <button className="p-4 border border-slate-600 rounded-lg hover:bg-slate-700 text-left">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h4 className="font-medium text-white">Quick Replies</h4>
                <p className="text-sm text-slate-300">Use pre-written response templates</p>
              </div>
            </div>
          </button>
          <button className="p-4 border border-slate-600 rounded-lg hover:bg-slate-700 text-left">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h4 className="font-medium text-white">Archive Old</h4>
                <p className="text-sm text-slate-300">Archive messages older than 30 days</p>
              </div>
            </div>
          </button>
        </div>
