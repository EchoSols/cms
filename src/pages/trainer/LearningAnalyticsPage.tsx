...
import { TrendingUp, Users, Clock, Award, CheckCircle, Target, Activity } from "lucide-react"
    ...
    <div className="p-6 space-y-6 bg-slate-900 min-h-screen">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-300">Comprehensive insights into learning performance and trends</p>
        </div>
        <select
          value={selectedPeriod}
          onChange={(e) => setSelectedPeriod(e.target.value)}
          className="px-3 py-2 bg-slate-800 border border-slate-600 text-white rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
        >
          ...
          <table className="min-w-full divide-y divide-slate-700">
            ...
            <tbody className="bg-slate-800 divide-y divide-slate-700">
              {filteredPrograms.map((program) => (
                <tr key={program.program} className="hover:bg-slate-700">
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm font-medium text-white">{program.program}</div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm text-white">{program.enrolled}</div>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm text-white">{program.avgScore}%</div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm text-white">{program.avgTime}</div>
                  </td>
                </tr>
              ))}
            </tbody>
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between p-3 bg-green-900/30 rounded-lg border border-green-800">
              <div className="flex items-center">
                ...
                <span className="text-sm font-medium text-green-300">Increasing Engagement</span>
              </div>
              <span className="text-lg font-bold text-green-400">+15%</span>
            </div>
            <div className="flex items-center justify-between p-3 bg-blue-900/30 rounded-lg border border-blue-800">
              <div className="flex items-center">
                <Clock className="w-5 h-5 text-blue-400 mr-2" />
                <span className="text-sm font-medium text-blue-300">Time to Complete</span>
              </div>
            ...
            <div className="flex items-center justify-between p-3 bg-yellow-900/30 rounded-lg border border-yellow-800">
              <div className="flex items-center">
                ...
                <span className="text-sm font-medium text-yellow-300">Goal Achievement</span>
              </div>
          ...
          <div className="space-y-4">
            <div className="flex items-start space-x-3">
              ...
              <div>
                ...
                <p className="text-sm text-slate-300">Data Analysis program shows highest completion rates</p>
              </div>
            </div>
            <div className="flex items-start space-x-3">
              <div className="w-2 h-2 bg-blue-500 rounded-full mt-2"></div>
              <div>
                <p className="text-sm font-medium text-white">Engagement Boost</p>
                <p className="text-sm text-slate-300">Interactive elements increased learner participation by 25%</p>
              </div>
            </div>
            <div className="flex items-start space-x-3">
              ...
              <div>
                ...
                <p className="text-sm text-slate-300">Agile Development program needs content optimization</p>
              </div>
            </div>
          </div>
        </div>
