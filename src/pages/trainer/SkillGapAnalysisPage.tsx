...
const SkillGapAnalysisPage = () => {
  ...
  const [selectedView, setSelectedView] = useState<
    "gaps" | "departments" | "trends"
  >("gaps");

  const filteredGaps = mockSkillGaps.filter((gap) => {
    const matchesSearch =
      gap.skillName.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  const criticalGaps = mockSkillGaps.filter(
    (g) => g.priority === "Critical"
  ).length;
  const highPriorityGaps = mockSkillGaps.filter(
    (g) => g.priority === "High"
  ).length;
  const totalCost = mockSkillGaps.reduce(
    (acc, gap) => acc + gap.estimatedCost,
    0
  );
  ...
  const getGapIndicator = (gap: number) => {
    if (gap > 1.5)
      return { icon: ArrowUp, color: "text-red-600", bg: "bg-red-100" };
    if (gap > 0.8)
      return { icon: ArrowUp, color: "text-orange-600", bg: "bg-orange-100" };
    if (gap > 0.3)
    ...
    <div className="p-6 bg-slate-900 min-h-screen">
      ...
      <div className="mb-8">
        ...
        <p className="text-slate-300">
          Identify skill gaps and plan targeted training interventions
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        <div className="bg-slate-800 p-6 rounded-lg border border-slate-700">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              <p className="text-sm font-medium text-slate-300">Total Gaps</p>
        ...
        <div className="bg-slate-800 p-6 rounded-lg border border-slate-700">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="text-2xl font-bold text-white">{criticalGaps}</p>
            </div>
          </div>
        </div>
              ...
              <div className="flex gap-3">
                <div className="relative">
                  ...
                  <input
                    ...
                    className="pl-10 pr-4 py-2 bg-slate-700 border border-slate-600 rounded-lg text-white placeholder-slate-400 focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                  />
                </div>
              ...
              <table className="min-w-full divide-y divide-slate-700">
                ...
                <tbody className="bg-slate-800 divide-y divide-slate-700">
                  {filteredGaps.map((gap) => {
                    ...
                    return (
                      <tr key={gap.id} className="hover:bg-slate-700">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="text-sm text-white">
                            {gap.currentLevel.toFixed(1)}/5.0
                          </div>
                        </td>

                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="text-sm text-white">
                            {gap.requiredLevel.toFixed(1)}/5.0
                          </div>
                        </td>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="flex items-center">
                            <div
                              className={`p-1 rounded-full ${gapIndicator.bg} mr-2`}
                            >
                              <gapIndicator.icon
                                className={`w-4 h-4 ${gapIndicator.color}`}
                              />
                            </div>
                            <span
                              className={`text-sm font-medium ${gapIndicator.color}`}
                            >
                              {gap.gap.toFixed(1)}
                            </span>
                          </div>
            ...
            >
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="text-right">
                  <div className="text-2xl font-bold text-white">
                    ${(dept.trainingBudget / 1000).toFixed(0)}k
                  </div>
                  <div className="text-sm text-slate-300">Training Budget</div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
                <div className="text-center">
                  ...
                  <div className="text-sm text-slate-300">Total Gaps</div>
                </div>
                <div className="text-center">
                  <div className="text-2xl font-bold text-red-400">
                    {dept.criticalGaps}
                  </div>
                  <div className="text-sm text-slate-300">Critical Gaps</div>
                </div>
                <div className="text-center">
                  <div className="text-2xl font-bold text-orange-400">
                    {dept.averageGap.toFixed(1)}
                  </div>
                  <div className="text-sm text-slate-300">Avg Gap</div>
                </div>
                <div className="text-center">
                  <div className="text-2xl font-bold text-green-400">
                    {dept.topSkills.length}
                  </div>
                  <div className="text-sm text-slate-300">Top Skills</div>
                </div>
              </div>

              <div>
                <h4 className="font-medium text-white mb-2">
                  Top Skills Needing Training:
                </h4>
                <div className="flex flex-wrap gap-2">
                  {dept.topSkills.map((skill, index) => (
                    <span
                      key={index}
                      className="inline-block px-3 py-1 bg-slate-700 text-sm text-slate-300 rounded-full"
                    >
                      {skill}
                    </span>
                  ))}
                ...
                <ul className="space-y-2">
                  <li className="flex items-start">
                    ...
                    <span className="text-sm text-slate-300">
                      Implement cybersecurity training for all technical staff
                    </span>
                  </li>
                  <li className="flex items-start">
                    <div className="w-2 h-2 bg-red-500 rounded-full mt-2 mr-3 flex-shrink-0"></div>
                    <span className="text-sm text-slate-300">
                      Launch data analysis certification program
                    </span>
                  </li>
                </ul>
              </div>
              ...
              <div>
                ...
                <ul className="space-y-2">
                  <li className="flex items-start">
                    ...
                    <span className="text-sm text-slate-300">
                      Develop leadership pipeline program
                    </span>
                  </li>
                  <li className="flex items-start">
                    <div className="w-2 h-2 bg-orange-500 rounded-full mt-2 mr-3 flex-shrink-0"></div>
                    <span className="text-sm text-slate-300">
                      Create digital marketing academy
                    </span>
                  </li>
                </ul>
              </div>
