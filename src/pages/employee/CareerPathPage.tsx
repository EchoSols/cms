  ...
  const skillsMatrix = {
    ...
    "Leadership Skills": {
      ...
      Communication: {
        current: 3,
        target: 4,
        skills: ["Technical Writing", "Presentations", "Stakeholder Management"],
      },
    ...
    return (
      <div className="flex items-center gap-1">
        {Array.from({ length: maxLevel }, (_, i) => {
          ...
          return <div key={i} className={`w-6 h-2 rounded-sm ${bgColor}`} />
        })}
      </div>
    )
  }
  ...
  return (
    <div className="p-6 bg-slate-900 min-h-screen">
      <div className="max-w-6xl mx-auto">
        {/* Header */}
        <div className="bg-gradient-to-r from-orange-600 to-orange-700 rounded-lg p-6 text-white mb-6">
          <h1 className="text-2xl font-bold mb-2">Career Development Path</h1>
          <p className="text-orange-100">Track your career progression and plan your next steps</p>
        </div>
        ...
        {activeTab === "current" && (
          <div className="space-y-6">
            ...
            <div className="bg-slate-800 border border-slate-700 p-6 rounded-lg">
              <h2 className="text-lg font-semibold text-white mb-6">Engineering Career Ladder</h2>
              <div className="space-y-6">
                      ...
                      <div className="flex items-start justify-between mb-4">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            ...
                            <p className="text-slate-300">{level.salary}</p>
                          </div>
                        </div>
                        <div className="flex items-center gap-2">
                          {level.status === "completed" && <CheckCircle className="w-5 h-5 text-green-400" />}
                          {level.status === "current" && (
                            <div className="text-sm text-blue-400 font-medium">{level.currentProgress}% Complete</div>
                          )}
                        </div>
                      </div>
                            ...
                            <div className="space-y-2">
                              {level.nextMilestones.map((milestone, idx) => (
                                <div key={idx} className="text-sm flex items-center gap-2">
                                  ...
                                  <span
                                    className={milestone.status === "completed" ? "text-green-300" : "text-slate-300"}
                                  >
                                    {milestone.title}
                                  </span>
                                </div>
                      ...
                      {level.status === "current" && level.currentProgress && (
                        <div className="mt-4">
                          <div className="flex justify-between items-center mb-1">
                            <span className="text-sm text-slate-400">Progress to Next Level</span>
                            <span className="text-sm font-medium text-white">{level.currentProgress}%</span>
                          </div>
        ...
        {activeTab === "skills" && (
          <div className="space-y-6">
            {Object.entries(skillsMatrix).map(([category, skills]) => (
              <div key={category} className="bg-slate-800 border border-slate-700 p-6 rounded-lg">
                <h2 className="text-lg font-semibold text-white mb-4">{category}</h2>
                <div className="space-y-4">
                  {Object.entries(skills).map(([skillName, skillData]) => (
                    <div key={skillName} className="border border-slate-600 rounded-lg p-4">
                      <div className="flex justify-between items-center mb-3">
                        <h3 className="font-medium text-white">{skillName}</h3>
                        <div className="text-sm text-slate-400">
                          Current: {skillData.current}/5 | Target: {skillData.target}/5
                        </div>
                      </div>
                      <div className="mb-3">{renderSkillBar(skillData.current, skillData.target)}</div>
                      <div className="flex flex-wrap gap-1">
                        {skillData.skills.map((skill, idx) => (
                          <span key={idx} className="bg-slate-700 text-slate-300 px-2 py-1 rounded-full text-xs">
                            {skill}
                          </span>
                        ))}
                      </div>
        ...
        {activeTab === "development" && (
          <div className="space-y-6">
            {developmentPlan.map((goal) => (
              <div key={goal.id} className="bg-slate-800 border border-slate-700 p-6 rounded-lg">
                <div className="flex justify-between items-start mb-4">
                  <div>
                    <h3 className="text-lg font-semibold text-white mb-1">{goal.goal}</h3>
                    <div className="flex items-center gap-3">
                      <span
                        className={`inline-flex items-center px-2 py-1 rounded-full text-xs ${getPriorityColor(goal.priority)}`}
                      >
                  ...
                  <div className="text-right">
                    <div className="text-2xl font-bold text-primary-400">{goal.progress}%</div>
                ...
                <div>
                  <h4 className="font-medium text-white mb-3">Activities</h4>
                  <div className="space-y-2">
                    {goal.activities.map((activity, idx) => (
                      <div key={idx} className="flex items-center justify-between p-3 bg-slate-700 rounded-lg">
                        <div className="flex items-center gap-3">
                          ...
                          <span className={activity.status === "completed" ? "text-green-300" : "text-slate-300"}>
                            {activity.name}
                          </span>
                        </div>
                        <div className="flex items-center gap-3">
                          ...
                          <span className="text-sm text-slate-400 w-8">{activity.progress}%</span>
                        </div>
                      </div>
                    ))}
                  </div>
