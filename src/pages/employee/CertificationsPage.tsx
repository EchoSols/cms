...
const CertificationsPage = () => {
  ...
  const earnedCertifications = [
    ...
    },
    {
      ...
      description:
        "Demonstrates proficiency in React development, including hooks, state management, and best practices.",
  ...
  return (
    <div className="p-6 bg-slate-900 min-h-screen">
      <div className="max-w-6xl mx-auto">
        {/* Header */}
        <div className="bg-gradient-to-r from-indigo-600 to-indigo-700 rounded-lg p-6 text-white mb-6">
          <h1 className="text-2xl font-bold mb-2">Certifications</h1>
          <p className="text-indigo-100">Track your professional certifications and credentials</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
          <div className="bg-slate-800 border border-slate-700 p-4 rounded-lg">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-sm text-slate-300">Total Earned</div>
              </div>
            </div>
          </div>
          <div className="bg-slate-800 border border-slate-700 p-4 rounded-lg">
          ...
          <div className="bg-slate-800 border border-slate-700 p-4 rounded-lg">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="text-xl font-bold text-white">{stats.expired}</div>
                <div className="text-sm text-slate-300">Expired</div>
              </div>
            </div>
          </div>
          <div className="bg-slate-800 border border-slate-700 p-4 rounded-lg">
        ...
        {earnedCertifications.some((c) => getExpiryStatus(c.expiryDate) === "expiring_soon") && (
          <div className="bg-yellow-900/50 border border-yellow-700 p-4 rounded-lg mb-6">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h3 className="font-semibold text-yellow-300 mb-2">Certifications Expiring Soon</h3>
                <div className="space-y-1">
                  {earnedCertifications
                    .filter((c) => getExpiryStatus(c.expiryDate) === "expiring_soon")
                    .map((cert) => (
                      <div key={cert.id} className="text-sm text-yellow-200">
        ...
        <div className="border-b border-slate-700 mb-6">
          <nav className="flex space-x-8">
            {[
              ...
              >
                {tab.label}
                <span className="bg-slate-700 text-slate-300 px-2 py-1 rounded-full text-xs">{tab.count}</span>
              </button>
            ))}
          </nav>
        </div>
        ...
        {activeTab === "earned" && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {earnedCertifications.map((cert) => {
              ...
              return (
                <div key={cert.id} className="bg-slate-800 border border-slate-700 p-6 rounded-lg">
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-slate-300 mb-2">{cert.issuingOrganization}</p>
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-4 text-sm">
                    ...
                    <div className="col-span-2">
                      ...
                      <span className="ml-2 font-medium font-mono text-xs text-slate-300">{cert.credentialId}</span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    <span
                      className={`inline-flex items-center px-2 py-1 rounded-full text-xs ${getStatusColor(expiryStatus || cert.status)}`}
                    >
                    ...
                    <div className="flex gap-2">
                      <button className="p-2 text-slate-400 hover:text-blue-400 transition-colors" title="View Details">
              ...
              <div key={cert.id} className="bg-slate-800 border border-slate-700 p-6 rounded-lg">
                <div className="flex justify-between items-start mb-4">
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-slate-300 mb-2">{cert.issuingOrganization}</p>
                    <p className="text-slate-400 mb-3">{cert.description}</p>
                  </div>
                </div>
                ...
                <div className="mb-4">
                  <div className="flex justify-between items-center mb-1">
                    ...
                    <span className="text-sm font-medium text-white">{cert.progress}%</span>
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4 text-sm">
                  <div>
                    ...
                    <span className="ml-2 font-medium text-white">
                      {new Date(cert.targetDate).toLocaleDateString()}
                    </span>
                  </div>
                  {cert.examDate && (
                    <div>
                ...
                <div>
                  <div className="text-sm text-slate-400 mb-2">Study Materials:</div>
                  <div className="space-y-1">
              ...
              <div key={cert.id} className="bg-slate-800 border border-slate-700 p-6 rounded-lg">
                ...
                <div className="grid grid-cols-2 gap-4 mb-4 text-sm">
                  ...
                  <div>
                    ...
                    <span className="ml-2 font-medium text-white">{cert.duration}</span>
                  </div>
                  <div>
                    <span className="text-slate-400">Cost:</span>
                    <span className="ml-2 font-medium text-white">${cert.cost}</span>
                  </div>
                  <div>
                    <span className="text-slate-400">Exam Code:</span>
                    <span className="ml-2 font-medium font-mono text-xs text-slate-300">{cert.examCode}</span>
                  </div>
                </div>

                <div className="mb-4">
                  <div className="text-sm text-slate-400 mb-2">Skills Gained:</div>
                  <div className="flex flex-wrap gap-1">
                    {cert.skills.map((skill, index) => (
                    ...
                    <ul className="space-y-1">
                      {cert.prerequisites.map((prereq, index) => (
                        <li key={index} className="text-sm text-slate-300 flex items-start gap-2">
                          <span className="w-1 h-1 bg-slate-500 rounded-full mt-2"></span>
                          {prereq}
                        </li>
                      ))}
                    </ul>
                ...
                <div className="flex justify-between items-center">
                  <div className="text-sm">
                    ...
                    <span className="ml-2 font-medium text-white">
                      {new Date(cert.nextExamDate).toLocaleDateString()}
                    </span>
                  </div>
        ...
        {showAddCertification && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-slate-800 border border-slate-700 rounded-lg shadow-xl w-full max-w-2xl mx-4 max-h-[90vh] overflow-y-auto">
              <div className="p-6">
                <h2 className="text-xl font-semibold text-white mb-4">Add Certification</h2>
                <form onSubmit={handleSubmitCertification} className="space-y-4">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full p-2 bg-slate-700 border border-slate-600 text-white rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent placeholder-slate-400"
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full p-2 bg-slate-700 border border-slate-600 text-white rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent"
                      />
                    </div>
                    <div>
                  ...
                  <div>
                    <label className="block text-sm font-medium text-slate-300 mb-1">Credential ID</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block text-sm font-medium text-slate-300 mb-1">Credential URL</label>
                    <input
                      type="url"
                  ...
                  <div>
                    <label className="block text-sm font-medium text-slate-300 mb-1">Upload Certificate</label>
